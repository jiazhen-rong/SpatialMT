...
vignette: >
  %\VignetteIndexEntry{SUMMIT Barrett's Esophagus Tutorial}
...
We can visualize such global significance results as in our paper in specified celltypes. 
 ...
 - `celltypes` could be set to be subtypes and the visualization will only show subtypes. 
 - `celltype_colors` allows customization of coloring.

```{r STEP2_result_visualization, include=T,eval=T,fig.width=10,fig.height=5}
# Visualize significance
celltype_colors = c("BE" = "#E00A82","SQ" = "#ea6860","IM" = "#538e8e","FB" = "#8bc3c3","VC" = "#a2b7d7")
celltypes = c("BE" ,"SQ" ,"IM","FB","VC")
p <- SpatialMT:::plot_lineage_significance(
  res = res_lg, celltypes=celltypes, fill_values=celltype_colors,
  order_by = "BE", title="Global Cell Type Test",
  outfile = file.path(save_path, "lineage_significance_allcelltype.pdf"), return_plot=T)
...
-->
<!--
<img src="example_data/results/lineage_significance_allcelltype.png" width="70%" />
-->

```{r STEP2_result_visualization_power, include=T,eval=T,fig.width=10,fig.height=5}
celltype_pattern = paste(celltypes,collapse = "|")
effect_sizes = seq(0.1, 1, by = 0.2)
p2<- SpatialMT:::plot_power_curves(
  beta_list = beta_list, effect_sizes = effect_sizes, 
  celltype_pattern=celltype_pattern, celltype_colors = celltype_colors,
  desired_order = c("3054_G>C","3071_T>C","15777_G>C"),
  save_pdf = TRUE, save_path = save_path, file_name = "Power_curve.pdf",return_plot=T)
print(p2)
```
<!--
...
-->
<!--
<img src="example_data/results/power_curve.png" width="70%" />
-->

### Step 3: Localized Cell Type Test

If a variant did not show up to be significantly associated with any celltype, it could be that they are too rare or with low heteroplasmy. However, a lot of rare clones are localized in space, so we have developed the paired localized test, hoping to discover the localized variants missed from global test.

The localized celltype test could be performed with fucntion `celltype_test_knn` (~ 3 min to calculate):
...
The raw p-values were plotted at spot-level and saved as pdf, and can be visualized as the following:

```{r model_local_visual_spot, include=T, eval=T,fig.width=15,fig.height=15}
var = "15777_G>C"
var_spot_res = readRDS(paste0(save_path,"/",var,"_spot_test.rds"))
print(var_spot_res$combined_plot)
```

Or summarized as bar plots, with minimum p-value for each celltype:
```{r model_local_visual_bars, include=T, eval=T,fig.width=5,fig.height=5}
celltype_colors = c("BE" = "#E00A82","SQ" = "#ea6860","IM" = "#538e8e","FB" = "#8bc3c3","VC" = "#a2b7d7")
celltypes = c("BE" ,"SQ" ,"IM","FB","VC")
spot_vis <- plot_localized_test(
  voi = c("15777_G>C"),
  save_path = "example_data/results/",
  Ws = Ws,
  signed = TRUE,
  celltypes = celltypes,
  all_ct=celltypes,
...
print(spot_vis$plot_all)
```



### Step 4: Other statistics
We also designed several new metrics, including LISI and MI. 
 - LISI measures the intermixing of two variants; 1 means fully separated and 2 means fully intermixed. 
 - Moran's I, which was first proposed for spatially variable DE genes, was calculated to check the coherence of the variances in space.

```{r step4_metric_lisi_MI,  eval=T}
### Step 0: Model Explanation (TODO)
LISI = pairwise_LISI(voi=voi,N=N,af.dm=af.dm,spatial_coords=spatial_coords,
                                  coverage_thresh=0,k = 50)
MI = moran_I_knn(voi=voi,N=N,af.dm=af.dm,spatial_coords=spatial_coords,
                             coverage_thresh=1,k=50,seed=42)
write.table(LISI,paste0(save_path,"/LISI_pairwise.txt"))
write.table(MI,paste0(save_path,"/MI_per_var.txt"))
```
