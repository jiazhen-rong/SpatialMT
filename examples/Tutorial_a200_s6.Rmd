...
library(SpatialMT)
library(SummarizedExperiment)  # <-- ADD THIS
library(SingleCellExperiment)  # often needed too
library(Matrix)
library(dplyr)
library(ggplot2)
library(Seurat)
...
```{r STEP2_result_visualization, include=T,eval=T}
...
```{r STEP2_result_visualization_power, include=T,eval=T}
...
```{r model_local_visual_spot, include=FALSE, eval=F}
var = "15777_G>C"
...
```{r model_local_visual_bars, include=FALSE, eval=F}
celltype_colors = c("BE" = "#E00A82","SQ" = "#ea6860","IM" = "#538e8e","FB" = "#8bc3c3","VC" = "#a2b7d7")
celltypes = c("BE" ,"SQ" ,"IM","FB","VC")
...
LISI aims to measure the 

```{r step4_metric_lisi_MI,  eval=F}
### Step 0: Model Explanation (TODO)
P3A2_LISI = pairwise_LISI(voi=voi,N=N,af.dm=af.dm,spatial_coords=spatial_coords,
                                  coverage_thresh=0,k = 50)
P3A2_MI = moran_I_knn(voi=voi,N=N,af.dm=af.dm,spatial_coords=spatial_coords,
                             coverage_thresh=1,k=50,seed=42)
write.table(P3A6_LISI_self,paste0(save_path,"/LISI_pairwise.txt"))
write.table(P3A6_LISI_self,paste0(save_path,"/MI_per_var.txt"))
```
